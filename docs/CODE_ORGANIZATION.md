...
1. Add domain types in `models.go` only if they are shared across multiple modules. Keep private, feature-only structs in that feature file.
2. Add schema changes in `db.go` and make migrations idempotent.
3. Add persistence functions in the owning module or a clearly named persistence file.
4. Add parser/calculation code as pure functions when possible.
5. Add HTTP handlers in the owning module and register routes in `app.go`.
6. Add CLI commands in `cli.go` only when the feature needs command-line access.
7. Add focused tests next to the behavior being changed.
8. Update `README.md` only for user-facing behavior.
